...
bool run_test(void)
{
    ...
    size_t tx_rand_delay = (997 * 8) - 369;//gsl_rng_uniform_int(rng, SCF_SYM_LEN) + START_OFFSET * SCF_SYM_LEN;
    float tx_rand_freq_offset = 0.0f;//(gsl_rng_uniform(rng) - 0.5f) * 2.0f * 200.0f;
    ...
    add_awgn(noise, RX_SIGNAL_LEN, 1.21f);
